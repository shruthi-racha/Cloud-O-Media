...
public class new1 implements ActionListener
{
	...
	new1(String username,String type)
	{
		try
		{
	  ...
	  jp.add(jb);
		...
		jf.setVisible(true);
			...
			else if(type.equals("2"))
			{
				try
				{
					PreparedStatement ps=con.prepareStatement("select * from log_details ");
			...
			else
			{
				try
				{
					PreparedStatement ps=con.prepareStatement("select * from log_details where bucket=?");
					ps.setString(1, username);
					ResultSet rs=ps.executeQuery();
					String a="";
					while(rs.next())
